...
class App {
  ...
  setupThreeJs() {
    ...
    /** Initialize our demo scene. */
     this.scene = DemoUtils.createLitScene(); //마커 
     
     this.reticle = new Reticle(); ///마커
     this.scene.add(this.reticle); //마커
  ...
  /** Place a sunflower when the screen is tapped. */
   onSelect = () => {
     if (window.heritage) {
      ...
      //  const shadowMesh = this.scene.children.find(c => c.name === 'shadowMesh');// 그림자 
      //  shadowMesh.position.y = clone.position.y;//그림자 
     }
   }

   deleteClone = () => {
      // if(this.clones)this.scene.remove(this.clones);
